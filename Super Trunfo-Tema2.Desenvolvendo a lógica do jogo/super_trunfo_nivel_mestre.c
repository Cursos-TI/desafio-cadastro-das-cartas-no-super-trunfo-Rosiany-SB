#include <stdio.h>

int main() {
    /*
    Super Trunfo - Nível Mestre
    Comparação com múltiplos atributos e menus dinâmicos
    */

    // Dados das cartas (pré-cadastrados)
    char estado1 = 'A', estado2 = 'B';
    char codigo1[4] = "A01", codigo2[4] = "B02";
    char cidade1[50] = "São Paulo", cidade2[50] = "Rio de Janeiro";
    int populacao1 = 12300000, populacao2 = 6000000;
    float area1 = 1521.11, area2 = 1200.25;
    float pib1 = 699.28, pib2 = 300.50;
    int pontos_turisticos1 = 50, pontos_turisticos2 = 30;
    
    // Variáveis calculadas
    float densidade1 = populacao1 / area1;
    float densidade2 = populacao2 / area2;
    float pib_per_capita1 = pib1 / populacao1;
    float pib_per_capita2 = pib2 / populacao2;

    int opcao1, opcao2;
    float soma1 = 0, soma2 = 0;
    
    // Menu interativo - Primeiro atributo
    printf("=== SUPER TRUNFO - COMPARAÇÃO AVANÇADA ===\n\n");
    printf("Cartas disponíveis:\n");
    printf("1. %s (%c)\n", cidade1, estado1);
    printf("2. %s (%c)\n\n", cidade2, estado2);
    
    printf("Escolha o PRIMEIRO atributo para comparação:\n");
    printf("1 - População\n");
    printf("2 - Área\n");
    printf("3 - PIB\n");
    printf("4 - Pontos Turísticos\n");
    printf("5 - Densidade Demográfica\n");
    printf("6 - PIB per capita\n");
    printf("\nDigite sua opção: ");
    scanf("%d", &opcao1);

    // Limpar buffer do teclado
    while(getchar() != '\n');

    // Menu dinâmico - Segundo atributo (exclui a opção já escolhida)
    printf("\nEscolha o SEGUNDO atributo (diferente do primeiro):\n");
    
    // Mostra apenas as opções não selecionadas
    for(int i = 1; i <= 6; i++) {
        if(i != opcao1) {
            switch(i) {
                case 1: printf("1 - População\n"); break;
                case 2: printf("2 - Área\n"); break;
                case 3: printf("3 - PIB\n"); break;
                case 4: printf("4 - Pontos Turísticos\n"); break;
                case 5: printf("5 - Densidade Demográfica\n"); break;
                case 6: printf("6 - PIB per capita\n"); break;
            }
        }
    }
    
    printf("\nDigite sua opção: ");
    scanf("%d", &opcao2);

    // Limpar buffer do teclado
    while(getchar() != '\n');

    // Verifica se o usuário digitou a mesma opção duas vezes
    if(opcao1 == opcao2) {
        printf("\nErro: Você selecionou o mesmo atributo duas vezes!\n");
        return 1;
    }

    printf("\n=== RESULTADO DA COMPARAÇÃO ===\n");
    printf("Carta 1: %s (%c)\n", cidade1, estado1);
    printf("Carta 2: %s (%c)\n\n", cidade2, estado2);

    // Comparação do primeiro atributo
    printf("Primeiro atributo: ");
    switch(opcao1) {
        case 1: // População
            printf("POPULAÇÃO\n");
            printf("%s: %d habitantes\n", cidade1, populacao1);
            printf("%s: %d habitantes\n", cidade2, populacao2);
            
            if(populacao1 > populacao2) {
                printf("Resultado: %s venceu neste atributo!\n", cidade1);
                soma1 += 1;
            } 
            else if(populacao2 > populacao1) {
                printf("Resultado: %s venceu neste atributo!\n", cidade2);
                soma2 += 1;
            } 
            else {
                printf("Resultado: Empate neste atributo!\n");
                soma1 += 0.5;
                soma2 += 0.5;
            }
            break;
            
        case 2: // Área
            printf("ÁREA\n");
            printf("%s: %.2f km²\n", cidade1, area1);
            printf("%s: %.2f km²\n", cidade2, area2);
            
            if(area1 > area2) {
                printf("Resultado: %s venceu neste atributo!\n", cidade1);
                soma1 += 1;
            } 
            else if(area2 > area1) {
                printf("Resultado: %s venceu neste atributo!\n", cidade2);
                soma2 += 1;
            } 
            else {
                printf("Resultado: Empate neste atributo!\n");
                soma1 += 0.5;
                soma2 += 0.5;
            }
            break;
            
        case 3: // PIB
            printf("PIB\n");
            printf("%s: %.2f bilhões\n", cidade1, pib1);
            printf("%s: %.2f bilhões\n", cidade2, pib2);
            
            if(pib1 > pib2) {
                printf("Resultado: %s venceu neste atributo!\n", cidade1);
                soma1 += 1;
            } 
            else if(pib2 > pib1) {
                printf("Resultado: %s venceu neste atributo!\n", cidade2);
                soma2 += 1;
            } 
            else {
                printf("Resultado: Empate neste atributo!\n");
                soma1 += 0.5;
                soma2 += 0.5;
            }
            break;
            
        case 4: // Pontos Turísticos
            printf("PONTOS TURÍSTICOS\n");
            printf("%s: %d pontos\n", cidade1, pontos_turisticos1);
            printf("%s: %d pontos\n", cidade2, pontos_turisticos2);
            
            if(pontos_turisticos1 > pontos_turisticos2) {
                printf("Resultado: %s venceu neste atributo!\n", cidade1);
                soma1 += 1;
            } 
            else if(pontos_turisticos2 > pontos_turisticos1) {
                printf("Resultado: %s venceu neste atributo!\n", cidade2);
                soma2 += 1;
            } 
            else {
                printf("Resultado: Empate neste atributo!\n");
                soma1 += 0.5;
                soma2 += 0.5;
            }
            break;
            
        case 5: // Densidade Demográfica (menor vence)
            printf("DENSIDADE DEMOGRÁFICA (menor vence)\n");
            printf("%s: %.2f hab/km²\n", cidade1, densidade1);
            printf("%s: %.2f hab/km²\n", cidade2, densidade2);
            
            if(densidade1 < densidade2) {
                printf("Resultado: %s venceu neste atributo!\n", cidade1);
                soma1 += 1;
            } 
            else if(densidade2 < densidade1) {
                printf("Resultado: %s venceu neste atributo!\n", cidade2);
                soma2 += 1;
            } 
            else {
                printf("Resultado: Empate neste atributo!\n");
                soma1 += 0.5;
                soma2 += 0.5;
            }
            break;
            
        case 6: // PIB per capita
            printf("PIB PER CAPITA\n");
            printf("%s: %.8f bilhões/hab\n", cidade1, pib_per_capita1);
            printf("%s: %.8f bilhões/hab\n", cidade2, pib_per_capita2);
            
            if(pib_per_capita1 > pib_per_capita2) {
                printf("Resultado: %s venceu neste atributo!\n", cidade1);
                soma1 += 1;
            } 
            else if(pib_per_capita2 > pib_per_capita1) {
                printf("Resultado: %s venceu neste atributo!\n", cidade2);
                soma2 += 1;
            } 
            else {
                printf("Resultado: Empate neste atributo!\n");
                soma1 += 0.5;
                soma2 += 0.5;
            }
            break;
            
        default:
            printf("Opção inválida para o primeiro atributo!\n");
            return 1;
    }

    printf("\n");

    // Comparação do segundo atributo
    printf("Segundo atributo: ");
    switch(opcao2) {
        case 1: // População
            printf("POPULAÇÃO\n");
            printf("%s: %d habitantes\n", cidade1, populacao1);
            printf("%s: %d habitantes\n", cidade2, populacao2);
            
            if(populacao1 > populacao2) {
                printf("Resultado: %s venceu neste atributo!\n", cidade1);
                soma1 += 1;
            } 
            else if(populacao2 > populacao1) {
                printf("Resultado: %s venceu neste atributo!\n", cidade2);
                soma2 += 1;
            } 
            else {
                printf("Resultado: Empate neste atributo!\n");
                soma1 += 0.5;
                soma2 += 0.5;
            }
            break;
            
        case 2: // Área
            printf("ÁREA\n");
            printf("%s: %.2f km²\n", cidade1, area1);
            printf("%s: %.2f km²\n", cidade2, area2);
            
            if(area1 > area2) {
                printf("Resultado: %s venceu neste atributo!\n", cidade1);
                soma1 += 1;
            } 
            else if(area2 > area1) {
                printf("Resultado: %s venceu neste atributo!\n", cidade2);
                soma2 += 1;
            } 
            else {
                printf("Resultado: Empate neste atributo!\n");
                soma1 += 0.5;
                soma2 += 0.5;
            }
            break;
            
        case 3: // PIB
            printf("PIB\n");
            printf("%s: %.2f bilhões\n", cidade1, pib1);
            printf("%s: %.2f bilhões\n", cidade2, pib2);
            
            if(pib1 > pib2) {
                printf("Resultado: %s venceu neste atributo!\n", cidade1);
                soma1 += 1;
            } 
            else if(pib2 > pib1) {
                printf("Resultado: %s venceu neste atributo!\n", cidade2);
                soma2 += 1;
            } 
            else {
                printf("Resultado: Empate neste atributo!\n");
                soma1 += 0.5;
                soma2 += 0.5;
            }
            break;
            
        case 4: // Pontos Turísticos
            printf("PONTOS TURÍSTICOS\n");
            printf("%s: %d pontos\n", cidade1, pontos_turisticos1);
            printf("%s: %d pontos\n", cidade2, pontos_turisticos2);
            
            if(pontos_turisticos1 > pontos_turisticos2) {
                printf("Resultado: %s venceu neste atributo!\n", cidade1);
                soma1 += 1;
            } 
            else if(pontos_turisticos2 > pontos_turisticos1) {
                printf("Resultado: %s venceu neste atributo!\n", cidade2);
                soma2 += 1;
            } 
            else {
                printf("Resultado: Empate neste atributo!\n");
                soma1 += 0.5;
                soma2 += 0.5;
            }
            break;
            
        case 5: // Densidade Demográfica (menor vence)
            printf("DENSIDADE DEMOGRÁFICA (menor vence)\n");
            printf("%s: %.2f hab/km²\n", cidade1, densidade1);
            printf("%s: %.2f hab/km²\n", cidade2, densidade2);
            
            if(densidade1 < densidade2) {
                printf("Resultado: %s venceu neste atributo!\n", cidade1);
                soma1 += 1;
            } 
            else if(densidade2 < densidade1) {
                printf("Resultado: %s venceu neste atributo!\n", cidade2);
                soma2 += 1;
            } 
            else {
                printf("Resultado: Empate neste atributo!\n");
                soma1 += 0.5;
                soma2 += 0.5;
            }
            break;
            
        case 6: // PIB per capita
            printf("PIB PER CAPITA\n");
            printf("%s: %.8f bilhões/hab\n", cidade1, pib_per_capita1);
            printf("%s: %.8f bilhões/hab\n", cidade2, pib_per_capita2);
            
            if(pib_per_capita1 > pib_per_capita2) {
                printf("Resultado: %s venceu neste atributo!\n", cidade1);
                soma1 += 1;
            } 
            else if(pib_per_capita2 > pib_per_capita1) {
                printf("Resultado: %s venceu neste atributo!\n", cidade2);
                soma2 += 1;
            } 
            else {
                printf("Resultado: Empate neste atributo!\n");
                soma1 += 0.5;
                soma2 += 0.5;
            }
            break;
            
        default:
            printf("Opção inválida para o segundo atributo!\n");
            return 1;
    }

    // Resultado final
    printf("\n=== RESULTADO FINAL ===\n");
    printf("Pontuação %s: %.1f\n", cidade1, soma1);
    printf("Pontuação %s: %.1f\n\n", cidade2, soma2);
    
    if(soma1 > soma2) {
        printf("Vencedor final: %s!\n", cidade1);
    } 
    else if(soma2 > soma1) {
        printf("Vencedor final: %s!\n", cidade2);
    } 
    else {
        printf("Resultado final: Empate!\n");
    }

    return 0;
}